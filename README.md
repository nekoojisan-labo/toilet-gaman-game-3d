...
- 主人公と通行人4種（サラリーマン・OL・学生・帽子のおじさん）を共通Humanoidリグの`THREE.SkinnedMesh`で描画
  - スキンウェイト（支配ボーン）＋バインドポーズ座標による頂点カラー塗装でスーツ・シャツ・ネクタイ・髪・肌を塗り分け
  - ブリーフケース・ハンドバッグ・帽子はボーン直付けで歩行アニメに追従
...
- カメラと主人公の間に入る設備の透過処理
- 迷路探索型のゴール: トイレの位置は毎ラン抽選され、扉を目視で「発見」するまで到達ゲージは？？？表示
- 分岐に設置された緑の案内板に近づくと、トイレの方角ヒント（HUDメッセージ＋頭上の3D矢印）が出る
- 制限時間、我慢ゲージ、発見後に有効化されるトイレ到達ゲージ
- タイトル画面はキーアート（`assets/sprites/keyart-title.jpg`）＋Ken Burns・段階フェードのアニメーション構成
