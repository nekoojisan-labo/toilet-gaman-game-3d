...
function addZoneObstacle(group, stage, x, z, accentColor) {
  ...
  if (stage.zone === "platform") {
    if (z === 6 && x >= 9 && x <= 12) {
      ...
      const joinsX = isWall(stage, x - 1, z) || isWall(stage, x + 1, z);
      const joinsZ = isWall(stage, x, z - 1) || isWall(stage, x, z + 1);
      const addPartitionX = () => {
        addBox(obstacle, new THREE.BoxGeometry(CELL * 0.96, 1.28, 0.22), pale, 0, 0.64, 0);
        addBox(obstacle, new THREE.BoxGeometry(CELL * 0.96, 0.10, 0.28), glow, 0, 1.31, 0);
      };
      const addPartitionZ = () => {
        addBox(obstacle, new THREE.BoxGeometry(0.22, 1.28, CELL * 0.96), pale, 0, 0.64, 0);
        addBox(obstacle, new THREE.BoxGeometry(0.28, 0.10, CELL * 0.96), glow, 0, 1.31, 0);
      };
      if (joinsX) addPartitionX();
      if (joinsZ) addPartitionZ();
      if (!joinsX && !joinsZ) {
        addBox(obstacle, new THREE.BoxGeometry(1.72, 0.82, 0.72), dark, 0, 0.41, 0);
        addBox(obstacle, new THREE.BoxGeometry(1.62, 0.10, 0.82), glow, 0, 0.87, 0);
      }
      addBox(obstacle, new THREE.BoxGeometry(0.24, 1.48, 0.24), steel, 0, 0.74, 0);
    }
    return true;
  }
...
function addRiggedCharacterStyle(root, type, height, style) {
  ...
  if (type === "player") {
    ...
    // 背中の大型バッグは削除。白い襟とスーツの腰回りだけで服装を読ませる。
    box(0.24, 0.055, 0.035, shirtMat, 0, height * 0.755, -0.155);
    box(0.38, 0.075, 0.30, jacketMat, 0, height * 0.455, 0);
  } else if (type === "business") {
    box(0.38, 0.34, 0.12, accentMat, 0.42, height * 0.31, 0.02);
    box(0.38, 0.07, 0.29, jacketMat, 0, height * 0.455, 0);
  } else if (type === "ol") {
    const skirt = addPart(new THREE.CylinderGeometry(0.27, 0.36, 0.46, 8), jacketMat, 0, height * 0.43, 0);
    skirt.scale.z = 0.72;
    const backHair = box(0.27, 0.32, 0.09, hairMat, 0, height * 0.84, -0.12);
    backHair.rotation.x = -0.10;
    box(0.21, 0.26, 0.12, accentMat, -0.42, height * 0.62, 0.04);
  } else if (type === "student") {
    box(0.32, 0.39, 0.14, accentMat, 0, height * 0.62, -0.20);
    [-0.10, 0.10].forEach((x) => box(0.035, 0.36, 0.025, accentMat, x, height * 0.64, -0.29));
  } else if (type === "traveler") {
    box(0.43, 0.075, 0.31, accentMat, 0, height * 0.49, 0);
...
function startStage(index) {
  ...
  for (let i = 0; i < stage.enemyCount; i++) {
    ...
    const obj = createCharacter(typeKey, typeKey === "traveler" ? 1.88 : 1.82);
...
function updateWallVisibility(stage, playerWorld) {
  ...
  for (const obstacle of zoneObstacles) {
    ...
    const dCamera = Math.hypot(wx - cameraWorld.x, wz - cameraWorld.z);
    const dSight = pointSegmentDistance2D(wx, wz, cameraWorld.x, cameraWorld.z, playerWorld.x, playerWorld.z);
    // 経路を作る駅設備を消すと迷路そのものが見えなくなるため、
    // カメラへ極端に接近した設備だけを非表示にする。
    obstacle.group.visible = dCamera > 1.15;
  }
}
